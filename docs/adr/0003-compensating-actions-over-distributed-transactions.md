...
- **Correction:** the "sweep job as a backstop" line above originally pointed at the wrong state. `ExpiredHoldsSweepJob` only ever looks at `status = 'held'` rows - it was never capable of catching a hold left `booked` with no booking behind it, which is exactly the leftover state a *failed* compensation (not merely an exception, one that never runs at all - a process crash between `HoldConfirmation.ConfirmHoldAsync`'s write and the `Booking` insert in `ConfirmBookingHandler`) actually produces. `Bookings.Jobs.ReconcileOrphanedBookedHoldsJob` is the real backstop for that state: it asks Catalog (via `IHoldLookup.GetBookedHoldIdsOlderThanAsync`) for `'booked'` holds older than a grace period, checks which have no matching `bookings.hold_id` row, and releases the orphans through the existing `IHoldConfirmation.ReleaseHoldAsync`. Deliberately owned by Bookings, not Catalog - see [ADR-0004](0004-module-boundaries-via-contracts-projects.md)'s Consequences for why that module placement matters and how the boundary stays compiler-enforced.
