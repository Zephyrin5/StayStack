...
Ownership split:
- Each module owns its own jobs as `[TickerFunction]` methods against its own `DbContext` (`Bookings.Jobs.ExpiredHoldsSweepJob`, `Identity.Jobs.ExpiredRefreshTokensSweepJob`) - the module knows *what* should happen periodically.
...
- **A host can be registered without scheduling.** `App:Jobs:RunScheduler=false` keeps the operational store and the dashboard and starts no background service. Two callers want it: a second web replica, which has no reason to run the crons the first one runs (`docs/scale-out-findings.md`), and every integration test host, where a job's own commit is indistinguishable from the request under test to an injected fault ([ADR-0025](0025-retried-work-is-built-inside-the-retry.md)). `MeasurementsFixture` is the one test host that turns it back on, because `TwoHostSchedulerProbe` exists to watch two hosts share one cron.
