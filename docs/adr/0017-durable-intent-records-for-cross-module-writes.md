...
- **Deployment must be two releases.** Deleting the old jobs in the same release that introduces intents would strand every orphan created *before* the deploy: it has no intent row, so the new job cannot see it, and the old job is gone. Those holds would stay `booked` permanently. The old jobs therefore ship alongside the new one for one release, marked for deletion, with an integration test asserting both are safe to run together in either order. A backfill migration was rejected: synthesizing intent rows for existing orphans needs a raw `unit_availability_holds` ← `bookings` join inside a Bookings migration, which is the boundary violation ADR-0004 forbids and only works while the modules share a physical database. Note the overlap drains what the old jobs could *ever* reach - an orphan older than their 2-day window was already unreachable before this change and stays that way.
