...
- **Correction:** the telemetry-identity half of the justification above is currently moot, not just theoretical - `ConfigureObservabilityServices` (the call that registers `TelemetryPipelineBehavior` at all) is commented out in `Program.cs` ("Disabled until Grafana is configured"), confirmed by reading the file, not assumed. Nothing is currently collecting per-request-type telemetry at all. The trust-boundary argument (point 2 under Alternatives considered) is what's actually carrying this decision today, and is sufficient on its own - it doesn't depend on observability being wired up. Re-verify the telemetry argument actually holds once `ConfigureObservabilityServices` is uncommented, rather than assuming it still applies unchanged.
