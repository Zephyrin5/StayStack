...
**The record is therefore a credential for its window, even though it stores none.** `checkout_idempotency_records` holds hashes and a booking id, and a database reader learns nothing usable from it; but whoever holds the *key* can mint a fresh, fully live management token on demand, for as long as the window lasts. The row is not the credential - the path is. Two things bound what that path can leave behind:

- **A cap of `BookingManagementTokenConfiguration.MaxLivePerBooking` live tokens per booking.** Past it, minting evicts the oldest by `created_at`. Without one, the number of live credentials for a booking is chosen by whoever replays, and each survives until the booking's own window closes months later.
- **Eviction, not refusal.** A cap that refused the mint would break the mechanism at exactly the moment it is needed: the client retrying is the case this ADR exists for. Evicting costs the holder of the oldest link, which is the trade taken deliberately - five is well above any honest retry count, so the guest who loses a link is the one whose key was replayed six times.

The mint and the eviction are one statement (`ConfirmBookingHandler.MintManagementTokenSql`), so a replay never deletes without inserting. It orders newest-first, so a token another replay committed a moment earlier is never the one evicted; two overlapping replays can leave one row over the cap, and the next replay takes it back down.

The 24-hour window (`BookingLifecyclePolicyOptions.CheckoutReplayWindowHours`) is checked in `ReplayAsync`, on the path that hands the credential over. `PurgeReplayedCheckoutsJob` deletes expired records as cleanup; enforcement does not depend on that job running. `ExpiredManagementTokensSweepJob` does the same for the tokens themselves once a booking is past `ManagementTokenLifetimeDaysAfterCheckOut`, using the booking's own timezone because `BookingAccessChecker` does ([ADR-0018](0018-property-local-business-dates.md)).
...
- **Several management tokens can exist per booking, up to the cap.** Each replay adds a row, so `booking_management_tokens.booking_id` has a plain index, not a unique one. The cap chooses eviction over refusal, and the cost is real: a guest holding the oldest of six links loses it, with no way to tell that link apart from one that never existed. The alternative was letting one key leave an unbounded number of live credentials behind, each good until the booking's own window closes.
- **The cap is enforced where tokens are minted, not by the schema.** A database constraint could only refuse the insert, which is the failure being avoided. `ManagementTokenCapTests` is what holds the number: removing the eviction leaves eight tokens where five are expected.
- **Neither job is load-bearing.** `ReplayAsync` enforces the replay window and `BookingAccessChecker` enforces the token's, both on the request path. The two sweeps bound table size and how long a dead credential lingers in it - a stopped job cannot extend anyone's access.
