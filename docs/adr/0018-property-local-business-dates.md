...
- **A globalization canary test** asserts real tzdata is present and that a DST-observing zone reports different offsets in January and July. `InvariantGlobalization` is unset; the test turns enabling it - for instance for a Native AOT publish ([ADR-0001](0001-native-aot-compatibility.md)) - into a test failure rather than an opaque 500 on cancellation.
