...
public record CancelBookingRequest : IRequest<CancelBookingResponse>
{
    ...
    ///     <para>
    ///         Cancelling is destructive and, until now, was reachable by
    ///         anyone who saw the link - a forwarded email, a screenshot, a
    ///         shared screen, a browser history on a family laptop. Viewing is
    ///         not: the management response deliberately carries no guest
    ///         email, so someone holding only the link cannot read this value
    ///         back out of the API. That asymmetry is what makes one form
    ///         field worth anything here - it turns a leaked link from
    ///         "cancel someone's holiday" into "read their itinerary".
    ///     </para>
    ///     <para>
