...
public class PropertyTests
{
    ...
    public void Create_ShouldSetAllProperties_WhenInputIsValid()
    {
        ...
        Property property = Property.Create(Guid.CreateVersion7(), hostId, PropertyType.Hotel, name, "Kuwait City", "Asia/Kuwait");
    ...
    {
        Property property = Property.Create(Guid.CreateVersion7(), Guid.NewGuid(), PropertyType.Chalet, CreateName(), null, "Asia/Kuwait");
    ...
    {
        Assert.ThrowsAny<ArgumentException>(() => Property.Create(Guid.CreateVersion7(), Guid.Empty, PropertyType.Hotel, CreateName(), null, "Asia/Kuwait"));
    }

    [Fact]
    public void Create_ShouldThrow_WhenNameIsNull()
    {
        Assert.Throws<ArgumentNullException>(() => Property.Create(Guid.CreateVersion7(), Guid.NewGuid(), PropertyType.Hotel, null!, null, "Asia/Kuwait"));
    }

    [Fact]
    public void Rename_ShouldUpdateName_WhenValid()
    {
        Property property = Property.Create(Guid.CreateVersion7(), Guid.NewGuid(), PropertyType.Hotel, CreateName(), null, "Asia/Kuwait");
